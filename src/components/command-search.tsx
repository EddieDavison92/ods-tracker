  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      try {
        ...
        if (id === seq.current) setItems(body.items ?? [])
      } catch {
  ...
  const trigger =
    variant === 'hero' ? (
      <button
        ...
        className="flex h-14 w-full items-center gap-3 rounded-2xl border border-white/15 bg-white px-5 text-left text-base text-muted-foreground shadow-xl shadow-black/20 transition hover:ring-4 hover:ring-white/20"
      >
        <Search aria-hidden className="h-5 w-5 text-primary" />
        <span className="flex-1 truncate">Search by name, ODS code or postcode…</span>
      ...
      <button
        type="button"
        onClick={() => setOpen(true)}
        className="flex h-9 w-full items-center gap-2 rounded-lg bg-white/10 px-3 text-left text-sm text-white/70 ring-1 ring-inset ring-white/15 transition hover:bg-white/15 hover:text-white"
      >
        <Search aria-hidden className="h-4 w-4" />
        <span className="flex-1 truncate">Search organisations…</span>
  ...
  return (
    <>
      ...
      <Command.Dialog
        ...
        contentClassName="fixed left-1/2 top-[12vh] z-50 w-[min(40rem,calc(100vw-2rem))] -translate-x-1/2 overflow-hidden rounded-2xl border bg-popover shadow-2xl"
      >
              ...
              <div className="flex flex-wrap gap-1.5">
                {EXAMPLES.map((ex) => (
                  <button
                    ...
                    type="button"
                    onClick={() => setQuery(ex)}
                    className="rounded-full border bg-card px-2.5 py-1 text-xs hover:border-primary/40 hover:bg-accent"
                  >
                    {ex}
                  </button>
                ))}
        ...
        <div className="flex items-center justify-between border-t bg-muted/50 px-4 py-2 text-[11px] text-muted-foreground">
          <span>Every ODS organisation type · open and closed</span>
